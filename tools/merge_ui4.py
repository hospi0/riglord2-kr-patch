# -*- coding: utf-8 -*-
"""`trans_ui4.TRANS`(반각 가타카나 대량 기술·무기·방어구명 210개)를 편입한다.

`merge_ui3.py` 와 완전히 같은 구조 — `ui_missing.scan()` 으로 다시 훑어서
`trans_ui4.TRANS` 에 있는 것만 `ui_strings.tsv`/`ui_refs.tsv` 에 추가한다.
"""
import hashlib
import io
import os
...
def main():
    ...
    iso = Iso(TRACK1)
    TRANS = trans_ui4.TRANS
    EXCLUDE = trans_ui4.EXCLUDE
