# -*- coding: utf-8 -*-
"""`trans_ui6.TRANS`(NUL 조각이 아닌 **데이터 표 안에 박힌** 문자열)를 편입한다.

`merge_ui5.py` 는 「NUL 구분 조각이되 너무 짧아서 필터에 걸린 것」을 다뤘고,
여기는 한 발 더 나가 **애초에 조각 경계가 안 잡히는 것**을 다룬다 — 앞에
0x00 없는 포인터 표가 붙어 있어 스캐너가 시작점을 못 찾는 경우.

찾는 방법 = 원문 바이트열 **정확 일치 + 바로 뒤가 0x00**(진짜 NUL 종단
문자열임을 확인). 14바이트 특정 패턴이라 오탐 위험은 사실상 없다.
"""
import hashlib
import io
import os
import sys

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from iso9660 import Iso
from mato import Mato
from project import TRACK1, WORK
from peek import load_list
import ui_corpus as U
import trans_ui6


def esc(s):
    return s.replace('\\', '\\\\').replace('\n', '\\n').replace('\r', '\\r').replace('\t', '\\t')


def find_exact(data, raw):
    """[off] — `raw` 와 정확히 일치하고 **바로 뒤가 NUL** 인 자리."""
    out = []
    i = 0
    while True:
        i = data.find(raw, i)
        if i < 0:
            break
        if i + len(raw) < len(data) and data[i + len(raw)] == 0:
            out.append(i)
        i += 1
    return out


def main():
    rows = {r[0]: r for r in load_list()}
    iso = Iso(TRACK1)
    TRANS = trans_ui6.TRANS

    new_str, new_ref, seen = [], [], set()

    def add(fnlabel, off, raw):
        jp, ko = TRANS[raw]
        key = hashlib.sha1(raw).hexdigest()[:10]
        if key not in seen:
            seen.add(key)
            new_str.append((key, len(raw), len(jp), jp, ko))
        new_ref.append((key, fnlabel, off, len(raw)))

    for raw in TRANS:
        for fn in U.OVL_FILES:
            _, lba, size, _ = rows[fn]
            m = Mato(iso.read(lba, size), fn)
            for bi, blk in enumerate(m.blocks()):
                for off in find_exact(bytes(blk), raw):
                    add('%s#%d' % (fn, bi), off, raw)

        for fn in U.PLAIN_FILES:
            _, lba, size, _ = rows[fn]
            for off in find_exact(bytes(iso.read(lba, size)), raw):
                add(fn, off, raw)

        _, lba, size, _ = rows[U.BIN_FILE]
        d = bytes(iso.read(lba, size))
        for off in find_exact(d, raw):
            if U.BIN_SAFE_LO <= off <= U.BIN_SAFE_HI:
                add(U.BIN_FILE, off, raw)

    sp = os.path.join(WORK, 'trans', 'ui_strings.tsv')
    rp = os.path.join(WORK, 'trans', 'ui_refs.tsv')
    have = set()
    with io.open(sp, encoding='utf-8') as f:
        next(f)
        for line in f:
            have.add(line.split('\t')[0])

    n1 = n2 = 0
    with io.open(sp, 'a', encoding='utf-8', newline='') as f:
        for key, nb, chars, jp, ko in new_str:
            if key in have:
                continue
            f.write('%s\t1\t%d\t%d\t\t0\t%s\t%s\n' % (key, nb, chars, esc(jp), esc(ko)))
            n1 += 1
    with io.open(rp, 'a', encoding='utf-8', newline='') as f:
        for key, fnlabel, off, nb in new_ref:
            if key in have:
                continue
            f.write('%s\t%s\t%d\t%d\n' % (key, fnlabel, off, nb))
            n2 += 1
    print('ui_strings.tsv +%d개 / ui_refs.tsv +%d건' % (n1, n2))
    for key, fnlabel, off, nb in new_ref:
        print('   %s 0x%X %dB' % (fnlabel, off, nb))
    missing = [k for k in TRANS if hashlib.sha1(k).hexdigest()[:10] not in seen]
    if missing:
        sys.exit('★디스크에서 못 찾은 항목: %s' % [TRANS[k] for k in missing])


if __name__ == '__main__':
    main()
